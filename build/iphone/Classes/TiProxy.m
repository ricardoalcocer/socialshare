...
//Rare exceptions to indicate a bug in the _socialshare code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_socialshare = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_socialshare]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.socialshare.module"
